...
Status updated September 17, 2026: real `gpt-6-astra` access and successful C-008/C-001 assessments are verified. Authentic sanitized captures were committed in `3c6483d`. Independent adversarial re-review closed the P0 claim-support and score-admissibility defects with PASS WITH P1 FIXES. This P1 pass aligns generation with the reviewed support catalog without changing validation. The under-150-second narrated recording has not yet been measured.
...
Historical P0-only state (superseded): the prompt did not supply that vocabulary,
so harmless novel paraphrases could be rejected. The P1 alignment below addresses
that generator/validator mismatch without expanding the accepted language.

### P1 generation alignment

`readiness/generation.py` builds compact request guidance from the canonical
`support_catalog.json` and existing support-policy constants for direct quotes.
It uses the policy's applicability predicates to select evidence-relevant claim
classes and sufficient supplied references, then checks candidate placements and
numeric options through the unchanged full validator. There is no second claim
allowlist, copied catalog, new scoring formula, or natural-language entailment layer.

Each option provides its output location, basis, exact statement/rationale pair,
references, required evidence types, numeric Data permissions, and human-review
requirement. Direct statements copy concrete deterministic evidence with the
policy's fixed rationale. Unsupported dimensions receive approved gap language.
Only applicable limitations and one reviewed wording variant per class are sent.
The two current fixtures' injected guidance is below 16,000 characters each.

The adapter appends these instructions to the existing safety prompt. Astra selects
claims rather than freely paraphrasing them; only the existing numeric Data-score
substitution is permitted. C-001 receives no numeric permission. The provider schema,
support catalog, support policy, evidence contract, scoring and local validator remain
unchanged. The validator is authoritative even if the model ignores the guidance.
Exact wording is a bounded MVP availability/expressiveness tradeoff, not a general
solution for arbitrary future CRM claims. New vocabulary still requires review.
...
The base safety prompt lives in `readiness/astra.py`; evidence-filtered controlled-language guidance is generated by `readiness/generation.py`. The adapter uses the OpenAI Responses API with `model=gpt-6-astra`, strict `text.format` JSON Schema, `store=false`, a 60-second timeout, a bounded response size, and no tools. `ASTRA_REASONING_EFFORT` defaults to `medium`; `low`, `medium`, and `high` are accepted. Redirects are rejected. Error messages never include provider bodies, request headers, key values, or exception text; only fixed/allowlisted codes leave the boundary.
...
runtime variability, not a proven deterministic UI/server defect. The P0 remediation
used no new live calls; the subsequent P1 pass separately verifies fresh generation.

The independent audit's original blockers were unsupported claims accepted with valid
...
Independent re-review subsequently closed both P0 defects and identified the
generation/validation mismatch addressed above. A measured under-150-second recording
and independent confirmation of the P1 change remain launch-verification steps.

### Fresh P1 verification — September 17, 2026

Exactly two fresh Responses API assessments used `gpt-6-astra`, reasoning `medium`,
the generated contract and the unchanged strict production validator. Both passed
schema, claim-support, reference, score-admissibility and human-authority checks on
the first attempt. No generation refinement, retry or validation relaxation occurred.

| Scenario | Service duration | Validated behavior |
| --- | --- | --- |
| C-008 | 28.90 seconds | Data 40 as a bounded qualitative estimate; overall and other dimensions null. Supervised review, conditional name verification, no autonomous recency decisions, and three evidence-ranked remediation priorities. |
| C-001 | 20.22 seconds | All scores null; no blockers or remediation invented. Supervised observation review and evidence acquisition; no detected issues explicitly distinguished from proven readiness. |

Both results used approved statement/rationale pairs, valid supplied references,
low confidence and explicit limitations; no invented business facts or authority
contradictions were observed. End-to-end CLI times were 29.13 and 20.44 seconds.
Non-secret verification references:

- C-008: `resp_0af800cb9a715175016aac07d7ff0887d2b7ac13ae816ce310`, completed `2026-09-17T15:32:03.238167+00:00`.
- C-001: `resp_0b537e5bf924f019016aac080dc9c487d287ec75dcbb396d58`, completed `2026-09-17T15:32:49.260079+00:00`.

These are two successful observations, not a repeatability or latency guarantee.
Historical demo captures were not replaced or modified. Offline verification passed
72 Python tests (63 existing plus nine alignment tests) and all three JavaScript
suites, including all 13 original P0 counterexamples. V1, policy, schemas and evidence
semantics remain unchanged. Exact-key and whitespace checks passed.
